...
/**
 * Created by 31344 on 2016/2/26. 读取医嘱线程
 */
public class ReadOrderThread extends Thread {
	...
	public ReadOrderThread(Date date, OrderReader reader,
			DrugReader drugReader, PatientInfoReader patientInfoReader,
			int interval) {
		this.lastDate = date;
		this.reader = reader;
		this.drugReader = drugReader;
		this.patientInfoReader = patientInfoReader;
	...
	public void run() {
		while (!exit) {
			...
			drugReader.readDrugList(lastDate, currentDate);
			patientInfoReader.readPatientInfoList(lastDate, currentDate);
	...
	private void writeLastReadTime(Date currentDate) {
		try {
			...
			BufferedWriter w = new BufferedWriter(new FileWriter(new File(path
					+ "/config/lastReadTime.txt"), false));
			w.write(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss")
					.format(currentDate));
			w.close();
		} catch (IOException e) {
