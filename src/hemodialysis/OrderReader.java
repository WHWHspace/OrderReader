...
/**
 * Created by 31344 on 2016/2/24. 读取医嘱实现
 */
public class OrderReader {
	...
	public static String password = "qqq63681048";
	...
	public void readNewAddedLongTermOrder(Date fromDate, Date toDate) {
		ArrayList<LongTermOrder> orders = hisImpl.getUpdatedLongTermOrder(
				fromDate, toDate);
	...
	public void readNewAddedShortTermOrder(Date fromDate, Date toDate) {
		ArrayList<ShortTermOrder> orders = hisImpl.getUpdatedShortTermOrder(
				fromDate, toDate);
	...
	/**
	 * 读取所有血透病人在his系统中的id id在输入病人基本信息的时候获取，保存在pif_insid
	 * 
	 * @return
	 */
		...
		if ((orders != null) && (orders.size() > 0)) {
			...
			for (int i = 0; i < orders.size(); i++) {
				...
				String sql = "INSERT INTO `shortterm_ordermgt` (`shord_patic`, `shord_dateord`, `shord_timeord`, `shord_usr1`, `shord_drug`,"
						...
						+ "('"
						+ order.getShord_patic()
						+ "', '"
						+ order.getShord_dateord()
						+ "', '"
						+ order.getShord_timeord()
						+ "', '"
						+ order.getShord_usr1()
						+ "', '"
						+ order.getShord_drug()
						+ "',"
						+ " '"
						+ order.getShord_actst()
						+ "', '"
						+ order.getShord_dtactst()
						+ "', '"
						+ order.getShord_usr2()
						+ "', '"
						+ order.getShord_comment()
						+ "',"
						+ " '"
						+ order.getShord_intake()
						+ "', '"
						+ order.getShord_freq()
						+ "', '"
						+ order.getShord_medway() + "');";
		...
		if ((orders != null) && (orders.size() > 0)) {
			...
			for (int i = 0; i < orders.size(); i++) {
				...
				String sql = "INSERT INTO `longterm_ordermgt` (`lgord_patic`, `lgord_dateord`, `lgord_timeord`, `lgord_usr1`, `lgord_drug`,"
						...
						+ "('"
						+ order.getLgord_patic()
						+ "', '"
						+ order.getLgord_dateord()
						+ "', '"
						+ order.getLgord_timeord()
						+ "', '"
						+ order.getLgord_usr1()
						+ "', '"
						+ order.getLgord_drug()
						+ "',"
						+ " '"
						+ order.getLgord_actst()
						+ "', '"
						+ order.getLgord_dtactst()
						+ "', '"
						+ order.getLgord_usr2()
						+ "', '"
						+ order.getLgord_comment()
						+ "',"
						+ " '"
						+ order.getLgord_intake()
						+ "', '"
						+ order.getLgord_freq()
						+ "', '"
						+ order.getLgord_medway() + "');";
				mysqlHelper.executeUpdate(sql);
			}
			mysqlHelper.closeConnection();
		}
