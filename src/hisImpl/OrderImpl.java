...
/**
 * Created by 31344 on 2016/2/24. 医嘱接口实现(本地数据库)
 */
public class OrderImpl implements OrderInterface {
			...
			while (rs.next()) {
				...
				try {
					...
					if (rs.getString("start_date_time") != null) {
						date = readFormat
								.parse(rs.getString("start_date_time"));
				...
				try {
					if (rs.getString("stop_date_time") != null) {
						Date stopDate = readFormat.parse(rs
								.getString("stop_date_time"));
				...
				o.setLgord_comment(rs.getString("comment")); // 备注
				String dosage = ((rs.getString("dosage") == null) ? "" : rs
						.getString("dosage"));
				String dosage_units = ((rs.getString("dosage_units") == null) ? ""
						: rs.getString("dosage_units"));
				o.setLgord_intake(dosage + dosage_units); // 剂量
				//o.setLgord_freq(rs.getString("lgord_freq")); // 频率
				o.setLgord_medway(rs.getString("medway")); // 用药方式
	...
	private ArrayList<ShortTermOrder> readShortTermOrderData(ResultSet rs) {
		...
		try {
			while (rs.next()) {
				...
				o.setShord_usr1(rs.getString("doctor")); // 医生
				String drugName = rs.getString("drug_name");
				...
				o.setShord_comment(rs.getString("comment")); // 备注
				String dosage = ((rs.getString("dosage") == null) ? "" : rs
						.getString("dosage"));
				String dosage_units = ((rs.getString("dosage_units") == null) ? ""
						: rs.getString("dosage_units"));
				...
				o.setShord_medway(rs.getString("medway")); // 用药方式
	...
	public ArrayList<LongTermOrder> getUpdatedLongTermOrder(Date fromDate,
		...
		SimpleDateFormat dateFormat = new SimpleDateFormat(
				"yyyy-MM-dd HH:mm:ss");
		String sql = "SELECT * from V_XT_PATIENT_ORDERS WHERE enter_date_time > to_date('"
				+ dateFormat.format(fromDate)
				+ "', 'yyyy-mm-dd hh24:mi:ss') and enter_date_time <= to_date('"
				+ dateFormat.format(toDate)
				+ "', 'yyyy-mm-dd hh24:mi:ss')  and repeat_indicator = '1'"
	...
	public ArrayList<ShortTermOrder> getUpdatedShortTermOrder(Date fromDate,
		...
		String sql = "SELECT * from V_XT_PATIENT_ORDERS WHERE enter_date_time > to_date('"
				...
				+ "', 'yyyy-mm-dd hh24:mi:ss') and enter_date_time <= to_date('"
				+ dateFormat.format(toDate)
				+ "', 'yyyy-mm-dd hh24:mi:ss') and repeat_indicator = '0'"
				+ "and PATIENT_ID in " + idsSql;
		ResultSet rs = helper.executeQuery(sql);
	...
	public ArrayList<DrugList> getUpdatedDrugList(Date fromDate, Date toDate) {
		...
		SimpleDateFormat dateFormat = new SimpleDateFormat(
				"yyyy-MM-dd HH:mm:ss");
		String sql = "SELECT DISTINCT DRUG_ID,DRUG_NAME FROM V_XT_PATIENT_ORDERS "
				+ "where enter_date_time > to_date('"
				+ dateFormat.format(fromDate)
				+ "', 'yyyy-mm-dd hh24:mi:ss') and enter_date_time <= to_date('"
				+ dateFormat.format(toDate) + "', 'yyyy-mm-dd hh24:mi:ss')";
		ResultSet rs = helper.executeQuery(sql);

		drugList = readDrugListData(rs);
		
		helper.closeConnection();
		return drugList;
	}

	private ArrayList<DrugList> readDrugListData(ResultSet rs) {
		ArrayList<DrugList> drugList = new ArrayList<DrugList>();
		if (rs == null) {
			return drugList;
		}
		try {
			...
			logger.error(new Date() + " 读取药品信息错误\n" + e);
			e.printStackTrace();
		}
		return drugList;
	}
	...
	public ArrayList<PatientInfomation> getUpdatedPatientInfomation(
		...
		SimpleDateFormat dateFormat = new SimpleDateFormat(
				"yyyy-MM-dd HH:mm:ss");
		String sql = "select DISTINCT PATIENT_ID,PATIENT_IC,PATIENT_NAME FROM V_XT_PATIENT_ORDERS"
				+ " where enter_date_time > to_date('"
				+ dateFormat.format(fromDate)
				+ "', 'yyyy-mm-dd hh24:mi:ss') and enter_date_time <= to_date('"
				+ dateFormat.format(toDate) + "', 'yyyy-mm-dd hh24:mi:ss')";
		...
		patientinfoList = readPatientinfoListData(rs);
	...
	private ArrayList<PatientInfomation> readPatientinfoListData(ResultSet rs) {
		ArrayList<PatientInfomation> patientinfoList = new ArrayList<PatientInfomation>();
		if (rs == null) {
			return patientinfoList;
		}
		try {
			while (rs.next()) {
				PatientInfomation p = new PatientInfomation();
				String patient_id = rs.getString("PATIENT_ID");
				String patient_ic = rs.getString("PATIENT_IC");
				String patient_name = rs.getString("PATIENT_NAME");
				p.setPatient_id(patient_id);
				p.setPatient_ic(patient_ic);
				p.setPatient_name(patient_name);
				patientinfoList.add(p);
			}
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			logger.error(new Date() + " 读取病人信息错误\n" + e);
			e.printStackTrace();
		}
		return patientinfoList;
	}
